...
class CurrencyRepository @Inject constructor(
    ...
    private fun fetchCurrencies(){
        ...
        if(request.isSuccessful){
            ...
            appDatabase.currencyDao().addCurrency(coinsWithExchange)
        }
    }
    ...
    suspend fun saveBaseCurrency(baseCurrencyCode : String) = withContext(Dispatchers.IO){
        prefs.edit().putString(SHARED_KEY_BASE_CURRENCY, baseCurrencyCode).apply()
    }

    suspend fun getBaseCurrencyPreference() : String = withContext(Dispatchers.IO){
        return@withContext prefs.getString(SHARED_KEY_BASE_CURRENCY, EMPTY_STRING) ?: EMPTY_STRING
    }
